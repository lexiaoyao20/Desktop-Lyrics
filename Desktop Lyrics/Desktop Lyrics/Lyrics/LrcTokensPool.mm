...
- (NSNumber *)parseTimeStamp:(NSString *)timeString
{
    ...
    while (anObject = [rTimeEnumrator nextObject]) {
	    lyricsTime += [anObject floatValue] * 1000 * nstep;
        nstep *= 60;
    }
...
- (NSString*)getLyricsByTime:(int)trackTime lyricsID:(NSUInteger *)lyid
{
	...
	if (lyricsCount > 0) {
		...
		if (left == right) {
            LyricItem *item = [lyricPool_ objectAtIndex:0];
            if (trackTime >= item.timeStamp) {
                if (lyid != NULL)
                    *lyid = left;
...
- (void)clearTokensPool {
    ...
    [lyricPool_ removeAllObjects];
}

- (BOOL)isLRCDidParsed:(NSString *)lrcPath {
    if ([lrcPath isEqualToString:path_]) {
        return YES;
    }
 ...
 * @param text 歌词ID fileDuration: 文件总时长 (单位为毫秒)
 * @result int 返回两者时间间隔
 */
- (int)timeDifferenceByID:(NSUInteger)lyid duration:(int)fileDuration { 
    if (!lyricPool_  || [self.lyrics count] == 0 || lyid > [self.lyrics count] - 1) {
        return -1;
    }
    
    if (lyid == [self.lyrics count] - 1) {
        LyricItem *item = [lyricPool_ objectAtIndex:lyid];
        return fileDuration - item.timeStamp;
    }
    
    LyricItem *currentItem = [lyricPool_ objectAtIndex:lyid];
    LyricItem *nextItem = [lyricPool_ objectAtIndex:lyid + 1];
    
    return nextItem.timeStamp - currentItem.timeStamp;
}
